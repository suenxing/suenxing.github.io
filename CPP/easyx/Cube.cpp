...
point p2p(double x, double y, double z); // 三维坐标变成三维点
dot p2d(point x, int choise);			 // 三维点映射成二维点
// choise是旋转模式（x,y,z三轴,可填1,2,3）
short sig(short x); // 1输出1，0输出-1
...
void Draw() // 绘制
{
	...
	{
		dot_table[i] = p2d(edge_table[i], METHOD); // 三维点转二维点
...
point p2p(double x, double y, double z)
{
	return (point){x, y, z};
}

dot p2d(point x, int choise) // 参考zhuanlan.zhihu.com/p/180551405
{
	if (choise == 1)
		return (dot){
